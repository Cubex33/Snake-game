...
impl Menu {
    pub fn draw() -> GameMenuAction {
        ...
        let shop_hover = mouse_x > btn_x && mouse_x < btn_x + btn_w 
            && mouse_y > settings_y && mouse_y < settings_y + btn_h;

        if draw_button(btn_x, start_y, btn_w, btn_h, "START GAME", start_hover) {
            return GameMenuAction::Start;
        }

        if draw_button(btn_x, settings_y, btn_w, btn_h, "SETTINGS", shop_hover) {
            return  GameMenuAction::Setting;
        }
